...
const SignUp = () => {
    ...
    const signUp = (e) => {
        ...
        .then((userCredential) => {
            ...
            setDoc(userInfoCollectionRef, {name: email, email: email,  password: password,
             coin: Number(0), addiction: "", level: Number(1), exp: Number(0),atk_dmg: Number(1), enemy_lev: Number(1),enemy_hp: Number(7)})
             //creates a document of user shop purchases on account creation
            setDoc(userShopCollectionRef, 
            {equipItem: "https://firebasestorage.googleapis.com/v0/b/letsquit-1b913.appspot.com/o/images%2FMini_Knight_Figure.webp?alt=media&token=c85094bc-dd58-4dbc-a79a-97e6511996da", 
            equipPet: "https://firebasestorage.googleapis.com/v0/b/letsquit-1b913.appspot.com/o/images%2FPet%2Fno_pet.avif?alt=media&token=16190947-db11-4f73-8ca1-8440134ffa6d",
             item1: Boolean(false), item2: Boolean(false), item3: Boolean(false), pet1: Boolean(false), pet2: Boolean(false), pet3: Boolean(false), pet4: Boolean(false) })
            navigate('/Home')
        })
        .catch((error) => {
            console.log(error);
